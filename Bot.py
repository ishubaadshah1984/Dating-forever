...
def reg_user(u):
    users.setdefault(u.id, {"name": u.first_name or "User", "anon": anon_name(u.id), "age": None, "city": None, "bio": None})
...
async def admin_panel_cb(update, context):
    ...
    await q.edit_message_text(f"Admin panel — {len(users)} users, {len(matches)} chats", reply_markup=InlineKeyboardMarkup(kb))
